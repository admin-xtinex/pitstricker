...
namespace PitStriker.EditorTools
{
    public static class VillageSceneSetup
    {
        ...
        public static void UpgradeBoundaryWalls()
        {
            ...
            // Boundary Walls: Strictly aligned with the authentic stacked stone wall border along the fairway
            // Left stone wall inner face is at X = -3.30m (center at -3.80m, thickness 1.0m, height 6.0m)
            // Right roadside border is at X = 6.0m (center at 6.50m, thickness 1.0m, height 6.0m)
            ConfigureWall("Wall_Left", new Vector3(-3.80f, 3.0f, 14.5f), new Vector3(1.0f, 6.0f, 52.0f), bouncePhys);
            ConfigureWall("Wall_Right", new Vector3(6.50f, 3.0f, 14.5f), new Vector3(1.0f, 6.0f, 52.0f), bouncePhys);
        ...
        private static void AddCollidersToVisualProps()
        {
            ...
            {
                bool isTargetProp = propPrefixes.Any(p => t.name.StartsWith(p)) ||
                                    (t.name.StartsWith("Detail_Stone_") && t.name.Contains("_-1_")) ||
                                    t.name.StartsWith("Detail_Plaster_-1") ||
                                    t.name == "Detail_Wood_0_-1" || t.name == "Detail_Wood_0_1";
                ...
                if (isTargetProp)
                {
                    ...
                    if (col == null)
                    {
                        ...
                        if (mf != null && mf.sharedMesh != null && mf.sharedMesh.vertexCount > 0 && mf.sharedMesh.vertexCount < 500)
                        {
