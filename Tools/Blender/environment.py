...
def _add_house(collection, wall_mat, roof_mat, wood_mat):
    ...
    roof_z = 3.05
    _add_box("Village_House_Roof_L", (-6.78, 13.0, roof_z), (2.15, 5.8, 0.20), roof_mat, collection, rotation=(0.0, math.radians(-25.0), 0.0))
    _add_box("Village_House_Roof_R", (-5.22, 13.0, roof_z), (2.15, 5.8, 0.20), roof_mat, collection, rotation=(0.0, math.radians(25.0), 0.0))
...
def _add_stone_wall(collection, stone_mat):
    ...
    for row in range(2):
        ...
        for i in range(18):
            ...
            dims = (random.uniform(0.45, 0.72), random.uniform(0.85, 1.18), random.uniform(0.32, 0.44))
            _add_box(f"StoneWall_{row}_{i}", (-4.55 + random.uniform(-0.08, 0.08), y, z), dims, stone_mat, collection, rotation=(0.0, 0.0, random.uniform(-0.08, 0.08)))
...
def _add_bicycle(collection, metal_mat):
    ...
    for idx, y in enumerate(wheel_y):
        bpy.ops.mesh.primitive_torus_add(major_radius=0.48, minor_radius=0.025, major_segments=24, minor_segments=6, location=(x, y, 0.55), rotation=(math.radians(90.0), 0.0, 0.0))
        wheel = bpy.context.object
        wheel.name = f"Bicycle_Wheel_{idx}"
        wheel.data.materials.append(metal_mat)
        _move_to_collection(wheel, collection)
    frame_points = {"rear": (x, wheel_y[0], 0.55), "front": (x, wheel_y[1], 0.55), "seat": (x, 9.62, 1.18), "pedal": (x, 9.78, 0.62), "bar": (x, 10.22, 1.18)}
    bars = [("Bike_Frame_1", "rear", "seat"), ("Bike_Frame_2", "seat", "pedal"), ("Bike_Frame_3", "pedal", "rear"), ("Bike_Frame_4", "pedal", "front"), ("Bike_Frame_5", "seat", "front"), ("Bike_Fork", "bar", "front")]
...
def _add_village_rough_soil(config, preset, collection):
    ...
    for i in range(100):
        ...
        _add_rock((x, y, s * 0.35), (s * random.uniform(1.2, 2.3), s, s * random.uniform(0.45, 0.85)), stone if i % 3 else stone_dark, collection, f"Village_Pebble_{i:03d}", subdivisions=1)
    ...
    palms = [(-7.0, -2.0, 4.8), (-6.8, 5.2, 5.3), (-7.2, 19.0, 5.0), (6.9, -1.0, 5.1), (7.2, 15.5, 5.5), (6.8, 27.0, 4.9), (-5.9, 26.5, 4.6), (4.8, 30.2, 4.4)]
...
def add_map_environment(config, preset):
    ...
    if config.map_name == "beach":
        from beach_shore import add_beach_shore
        add_beach_shore(config, preset, collection)
        ...
        for side in (-1, 1):
            _add_box(f"Clay_Curb_{'L' if side < 0 else 'R'}", (side * half_width * 0.88, config.pit_spacing, 0.13), (0.22, y_max - y_min, 0.26), clay_trim, collection)
    return collection
