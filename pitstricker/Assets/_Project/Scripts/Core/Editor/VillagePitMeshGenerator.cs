...
namespace PitStriker.EditorTools
{
    ...
    public static class VillagePitMeshGenerator
    {
        ...
        /// </summary>
        private static void ConfigureUnifiedArenaGround(Material soilMat, PhysicsMaterial physMat)
        {
            ...
            // 2. Generate Unified Full Arena Mesh with 3 Circular Pit Apertures
        ...
        /// (X in [-10, 10], Z in [-10, 38], Y=0.0m) with 3 exact circular holes (r=0.50m, 36 segments)
        /// at Z=3.0, 16.5, 31.0. All normals strictly point +Y (0, 1, 0) with zero inverted faces.
        /// </summary>
        private static Mesh GenerateUnifiedGroundMesh()
        {
            Mesh mesh = new Mesh();
            ...
            // Build the full unified arena ground:
            // Section 0: Z in [-10.0, 1.5]
            MakeGrid(-10.0f, 10.0f, -10.0f, 1.5f, 20, 10);

            // Pit 1 Zone: Z in [1.5, 4.5] (Center Z=3.0, halfSize=1.5)
            MakeGrid(-10.0f, -1.5f, 1.5f, 4.5f, 8, 3);
            MakeGrid(1.5f, 10.0f, 1.5f, 4.5f, 8, 3);
            MakePitBox(0.0f, 3.0f, 1.5f, 0.50f, 36);

            // Bridge 1-2: Z in [4.5, 15.0]
            MakeGrid(-10.0f, 10.0f, 4.5f, 15.0f, 20, 10);

            // Pit 2 Zone: Z in [15.0, 18.0] (Center Z=16.5, halfSize=1.5)
            MakeGrid(-10.0f, -1.5f, 15.0f, 18.0f, 8, 3);
            MakeGrid(1.5f, 10.0f, 15.0f, 18.0f, 8, 3);
            MakePitBox(0.0f, 16.5f, 1.5f, 0.50f, 36);

            // Bridge 2-3: Z in [18.0, 29.5]
            MakeGrid(-10.0f, 10.0f, 18.0f, 29.5f, 20, 10);

            // Pit 3 Zone: Z in [29.5, 32.5] (Center Z=31.0, halfSize=1.5)
            MakeGrid(-10.0f, -1.5f, 29.5f, 32.5f, 8, 3);
            MakeGrid(1.5f, 10.0f, 29.5f, 32.5f, 8, 3);
            MakePitBox(0.0f, 31.0f, 1.5f, 0.50f, 36);

            // Section End: Z in [32.5, 38.0]
            MakeGrid(-10.0f, 10.0f, 32.5f, 38.0f, 20, 6);
